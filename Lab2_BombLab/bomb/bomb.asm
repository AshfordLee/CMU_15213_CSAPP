...
0000000000400ee0 <phase_1>:
  ...
  //栈指针%rsp向下移动8个字节，栈空间增长8个字节
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi
  //将地址0x402400加载到%eax寄存器
  400ee9:	e8 4a 04 00 00       	call   401338 <strings_not_equal>
  //调用strings_not_equal比较两个字符串
  //前面有read_line函数把输入的字符串移动到%rdi，所以这是函数的一个参数
  //另一个根据x86-64约定是%esi(%rsi的低32位)
  //而%esi实际上是0x402400
  400eee:	85 c0                	test   %eax,%eax
  //同样的根据x86-64约定，一般返回值在%rax中，%eax是%rax的低32位
  //这里比较返回值是不是0
  400ef0:	74 05                	je     400ef7 <phase_1+0x17>
  //返回值是0那么根据je跳转到400ef7，如果不是0就爆炸
...
//所以，我们的输入应该是那个被保存到%esi里面的字符串，也就是一开始在%0x402400里面的字符串
...
0000000000400efc <phase_2>:
  ...
  //比较第一个数字(%rsp栈指针所指的数字)是不是1
  400f0e:	74 20                	je     400f30 <phase_2+0x34>
  //je表示如果等于1就跳转到400f30，否则炸弹爆炸
  400f10:	e8 25 05 00 00       	call   40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax
  //%rbx前4个字节的东西，那自然就是前一个数字，放到%eax里
  400f1a:	01 c0                	add    %eax,%eax
  //把%eax里面的数字乘以2
  400f1c:	39 03                	cmp    %eax,(%rbx)
  //(%rbx)是当前数字，检查当前数字是不是前一个数字的2倍(因为%eax已经被*2过了)
  400f1e:	74 05                	je     400f25 <phase_2+0x29>
  //不相等就炸弹爆炸，不等就跳转到400f25
  ...
  //到了边界就退出循环，没到边界就跳转到400f17继续循环
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp
  //注意%rsp指向第一个数字，所以这里是让%rbx指向第二个数字，%rbp指向第六个数字后面的位置
...
//所以是反复检查现在的这个数字是不是前一个的2倍，第一个数字是2
//所以应该输入1,2,4,8,16,32
...
0000000000400f43 <phase_3>:
  ...
  //调用scanf解析输入，看输入了多少个数字
  400f60:	83 f8 01             	cmp    $0x1,%eax
  //%eax作为scanf的返回值，只有当大于1(至少输入2个数字)的时候才不爆炸，跳转到400f6a
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	call   40143a <explode_bomb>
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp)
  //比较第一个输入的数字是不是<=7，大于7就爆炸
  //注意这里0x8是第一个读取到的数字，这是x86的约定
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a>
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax
  //把第一个数字放入%eax
  400f75:	ff 24 c5 70 24 40 00 	jmp    *0x402470(,%rax,8)
  //一个跳转表，为%eax设置特定值然后跳转到400fbe
  ...
  //比较%eax和第二个数字(x86约定0xc为scanf读取到的第二个数字)，不相等就爆炸
  400fc2:	74 05                	je     400fc9 <phase_3+0x86>
  400fc4:	e8 71 04 00 00       	call   40143a <explode_bomb>
  400fc9:	48 83 c4 18          	add    $0x18,%rsp
  400fcd:	c3                   	ret

//所以当输入0的时候，第二个数字要等于0xcf,也就是207
...
0000000000400fce <func4>:
  ...
  //%rax实际上是(high-low)/2,%rsi是low，所以算出来就是(high+low)/2
  //%ecx，计算%rax和%rsi的中点放到%ecx
...
000000000040100c <phase_4>:
  ...
  //检查第二个输入的数字是不是0，不是0就爆炸
  401056:	74 05                	je     40105d <phase_4+0x51>
  401058:	e8 dd 03 00 00       	call   40143a <explode_bomb>
  40105d:	48 83 c4 18          	add    $0x18,%rsp
  401061:	c3                   	ret

//func4其实是一个二分查找，下界是0，上界是14，所以传入7返回值就是0
...
0000000000401062 <phase_5>:
  ...
  //如果没处理6个字符就跳转到40108b(循环)
  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)
  //添加空字符
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi
  //把0x40245e的字符放入%esi(是我们想得到的字符串)
